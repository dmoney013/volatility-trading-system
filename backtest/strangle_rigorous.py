"""
Rigorous Strangle Backtester — proper train/validation/test split evaluation.

Strategy: GARCH-informed long strangle with configurable hold period.
Uses GARCH trained ONLY on the training set, tested on unseen data.

Mirrors backtest/rigorous.py but for the strangle strategy.
"""
import numpy as np
import pandas as pd
...
from config import (
    ...
    STRANGLE_OTM_WIDTH, MAX_POSITION_PCT, COMMISSION_PER_CONTRACT, TRADING_DAYS,
)
from backtest.rigorous import split_data, _get_risk_free, _market_iv, _build_results
from backtest.strangle_engine import (
    strangle_price, select_strangle_strikes, StrangleTrade,
)


# ═══════════════════════════════════════════════════════════════════
# Backtest — N-day hold long strangle
# ═══════════════════════════════════════════════════════════════════

def run_strangle_backtest(
    test_prices, cond_vol_test, treasury, ticker,
    initial_capital=INITIAL_CAPITAL, holding_period=STRANGLE_HOLDING_PERIOD_DAYS,
    entry_threshold=STRANGLE_ENTRY_VOL_THRESHOLD,
    otm_width=STRANGLE_OTM_WIDTH, verbose=True,
):
    """Run long strangle backtest on the test set with full hold period."""
    if verbose:
        print(f"\n{'='*60}")
        print(f"LONG STRANGLE: {holding_period}-day hold, {otm_width*100:.0f}% OTM")
        print(f"{'='*60}")
    ...
    for i in range(len(common_idx)):
        ...
        if in_position:
            if i - entry_idx >= holding_period:
                call_v = max(spot - entry_data["call_strike"], 0)
                put_v = max(entry_data["put_strike"] - spot, 0)
                exit_val = call_v + put_v
                trade, net = _close_strangle(entry_data, exit_val, spot, date, ticker, gvol, otm_width)
                trades.append(trade)
                capital += net
                in_position = False
        else:
            ...
            if spread > entry_threshold and i + holding_period < len(common_idx):
                entry_data = _open_strangle(spot, iv, gvol, r, date, capital, holding_period, spread, otm_width)
                if entry_data:
                    entry_idx = i
                    in_position = True

    return _build_strangle_results(trades, equity, capital, initial_capital, verbose)
...
def _open_strangle(spot, iv, gvol, r, date, capital, hold, spread, otm_width):
    T = hold / TRADING_DAYS
    call_strike, put_strike = select_strangle_strikes(spot, otm_width)
    price = strangle_price(spot, call_strike, put_strike, T, r, iv)
    if price <= 0.01:
        return None
    cost = price * 100
...
def _close_strangle(entry_data, exit_val, spot, date, ticker, gvol, otm_width):
    pnl_ps = exit_val - entry_data["entry_price"]
    pnl_total = pnl_ps * entry_data["contracts"] * 100
    comm = COMMISSION_PER_CONTRACT * 4  # 2 legs × (open + close)
    net = pnl_total - comm
    ...
    trade = StrangleTrade(
        ...
        call_strike=entry_data["call_strike"], put_strike=entry_data["put_strike"],
        entry_price=entry_data["entry_price"], exit_price=exit_val,
        contracts=entry_data["contracts"], entry_vol=entry_data["iv"],
        exit_vol=gvol, pnl_per_share=pnl_ps, pnl_total=pnl_total,
...
def _build_strangle_results(trades, equity, final_capital, initial_capital, verbose):
    """Build results dict from strangle trades. Reuses straddle stats logic."""
    equity_df = pd.DataFrame(equity) if equity else pd.DataFrame()
    if not trades:
        if verbose:
            print(f"\n⚠ No trades executed.")
        return {
            "total_trades": 0, "final_capital": final_capital,
    ...
    trades_df = pd.DataFrame([{
        ...
        "signal_strength": t.signal_strength, "otm_width": t.otm_width,
    } for t in trades])
...
def run_rigorous_strangle_evaluation(
    ticker, initial_capital=INITIAL_CAPITAL,
    ...
    Run full rigorous evaluation with 60/20/20 split:
      ...
      3. Backtest strangle on completely unseen test set
    """
    ...
    if verbose:
        print(f"\n{'#'*60}")
        print(f"RIGOROUS STRANGLE EVALUATION: {ticker}")
        print(f"{'#'*60}")
    ...
    results = run_strangle_backtest(
        test_prices, cond_vol_test, treasury, ticker,
        initial_capital=initial_capital, holding_period=holding_period,
        entry_threshold=entry_threshold, otm_width=otm_width,
        verbose=verbose,
    )
...
if __name__ == "__main__":
    # Test on a budget-friendly ticker
    result = run_rigorous_strangle_evaluation("F", holding_period=5)
    print(f"\nFinal: {result['results']['total_return_pct']:+.1f}% return")
